document.addEventListener("DOMContentLoaded", function () {
  ...
  function generarNotificaciones() {
    ...
    const librosMora = Array.from(
      document.querySelectorAll(".history-card")
    ).filter(
      (card) =>
        card.textContent.includes("Devolver") ||
        card.textContent.includes("Pendiente")
    );

    librosMora.forEach((card) => {
      const titulo =
        card.querySelector(".history-book-title")?.textContent ||
        "Libro sin título";
      const fecha =
        card.querySelector(".history-date")?.textContent ||
        "Fecha no disponible";
      list.innerHTML += `
    ...
    prestamos.forEach((card) => {
      const titulo =
        card.querySelector(".loan-book-title")?.textContent ||
        "Libro desconocido";
      const fechaDev =
        card.querySelector(".loan-return-date")?.textContent || "Sin fecha";
      list.innerHTML += `
        <div class="notification-item">
    ...
    const devueltos = Array.from(
      document.querySelectorAll(".history-card")
    ).filter((card) => card.textContent.includes("Devuelto"));
    devueltos.forEach((card) => {
      const titulo =
        card.querySelector(".history-book-title")?.textContent || "Libro";
      const fecha =
        card.querySelector(".history-date")?.textContent || "Sin fecha";
      list.innerHTML += `
        <div class="notification-item">
